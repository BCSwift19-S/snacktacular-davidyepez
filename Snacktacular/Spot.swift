...
//  Snacktacular
//
//  Created by David Yépez on 3/31/19.
        ...
        if self.documentID != "" {
            ...
            ref.setData(dataToSave) { (error) in
                if let error = error {
                    ...
                    print("^^^ ERROR: Document updated with ref ID \(ref.documentID)")
                    completion(true)
                }
            }
        } else {
            ...
            ref = db.collection("spots").addDocument(data: dataToSave) {error in
                if let error = error {
                    ...
                    print("^^^ ERROR: New Document created with ref ID \(ref?.documentID ?? "Unknown")")
                    completion(true)
                }
